...
> Naver 방지턱 연결(#5): 정확한 Naver type 22와 현재 양의 거리는 도로 분류가
> 없어도 공통 방지턱 제어기로 전달한다. 명시적 highway 0/1은 차단하고 Tmap 규칙은
> 유지한다. 없는 분류를 가짜 8로 바꾸지 않는다. 최신 공통 목표속도·종료 거리·gas
> 동작을 유지하며 합성 테스트 통과와 실제 주행 확인은 구분한다.

> 2026-09-24 bin9208 Naver 실험 브랜치: 7712 Naver/Tmap TCP와 기존 7713/7714
> 입력은 source snapshot 중재 후 최신 공용 CarrotServ로 전달한다. EOF는 해당
> 세션의 transport loss만 기록하고 lease 전에는 소유권을 지우지 않는다.
> 이번 단계는 소유권 연결이며 감속 폴백 정책 변경/실차 검증 완료가 아니다.
> [런타임 범위와 테스트 경계](naver/RUNTIME.md)를 참고한다.

> 후면단속 추가 처리: 외부 TMAP SDI 75/76은 마지막 50m 접근에서 위치를 기억하고
> `AutoNaviRearCameraHoldDistance`(기본 100m, 0~300m/10m 단위)만큼 통과 후에도 상한을 유지한다.
> 카운트다운은 원래 카메라 지점에서 끝나고, 화면의 후면단속 유지 거리는 별도로 표시한다.
> 공유 `CarrotServ`에서 7713/7714 모두 적용하며 순정 CAN에는 후면 종류를 추정해 적용하지 않는다.
> 세부 전환 조건은 [7714 비교 문서의 추가 처리](carrot_navi_7714_branch_comparison.md)를 참고한다.

> 2026-09-13 `carrot-wip` 및 유지 중인 `carrot-*` 변경: 외부 내비 연결 중에는 순정 내비를
> 감속·카운트다운·순정 내비 속도 표시에서 제외한다. 외부 안내 항목이 없어도 같은 우선순위를 유지하며,
> 연결 종료/수신 만료 후에만 순정 내비를 기존 설정에 따라 사용한다. 연결 판정과 전환 테스트는
> [7714 브랜치 비교](carrot_navi_7714_branch_comparison.md)의 2026-09-13 항목을 따른다.

> 2026-09-11 `carrot-wip` 변경: 공통 속도 선택에서 모델 미래속도 제한을 제거하고,
> 비전 커브 상한을 곡률과 남은 거리로 계산하도록 변경했다. 아래의 `navi-stream`
> 확인 기준과 과거 설정 표는 당시 동작을 설명한다. 현재 브랜치 차이와 검증 항목은
> [7714 브랜치 비교](carrot_navi_7714_branch_comparison.md)의 비전 커브 변경 항목을 따른다.
...
해석한다. A-CAN `0x380` byte 3 bit 6은 약 4.7초 동안 켜졌다 꺼지는 알림이 반복되어,
꺼짐이나 값 `0x04`를 카메라 통과 판정으로 사용하지 않는다. 현재 단속은 신선한
`0x364 MapSource=2`와 같은 제한속도의 가까운 거리 프로파일을 연결한다. 짧은 알림 종료만으로는
연결을 끊지 않지만, 단속 상태 종료·제한속도 변경·유효하지 않은 제한속도 또는 메시지 유실은
현재·대기 카메라를 모두 해제한다. 취소 시 이미 수신된 카메라 프로파일도 소비하여 재삽입을 막는다.
두 메시지는 올바른 길이·버스 상태와 1초 이내 수신을 모두 만족해야 한다.
PV5에는 현재 경로/위치 `0x4B9/0x4B4` 입력이 연결되지 않았으므로 기존 경로 초기화 코드를
경로 이탈 검출로 볼 수 없다. 순정 단속 상태가 같은 값으로 계속되면 별도로 이탈을 판별할 수 없다.

휠 주행거리로 현재 목표에 도달하면 해당 이벤트를 제거하고 같은 안내를 소비한다.
다음 거리 후보만으로 감속하지 않으며, 새 유효 안내와 새 거리 정보가 연결되어야 한다.
연결할 거리가 없으면 해당 안내의 가상거리를 사용한다. 가상거리 0은 통과 증거가 아니므로
안내가 유효한 동안 최솟값 1m를 유지한다. 단속 상태 종료·변경·유실 후에는 이전 거리를 복원하지 않는다.
PV5에서는 일반 카메라와 Value 6 방지턱을 지원하고, 종류 7 프로파일만으로 구간단속·
30 km/h 구간을 활성화하지 않는다. 카메라 취소는 방지턱 처리에 적용하지 않는다.

2026-09-21 후속 로그는 알림 종료 때 남은 거리 139m의 30 km/h 카메라가 삭제되고
1327m 앞 50 km/h 카메라로 바뀌는 오류를 재현했다. 이전 구간이 없어 초기 목록은 기록된
거리와 휠 주행거리에서 복원했으며, 506a68db 재생의 거리 최대 오차는 0.000066m였다.
이번 수정은 앞선 MapSource=1 구간에서 예전 거리 후보를 해제하므로, 신고 지점은 현재
30 km/h 안내의 가상거리로 처리한다. 54.703초에 거리 127.68m/감속 후보 53.98 km/h로
유지되며, 기존 50 km/h·1325.90m/186.30 km/h 후보로 바뀌지 않는다.
앞선 6구간의 거리 없는 50 km/h 안내도 재생 확인한다. 취소·복구·거리 소진 후 대기와
새 프로파일 수신은 합성 테스트로 검증하며 실제 경로 이탈·통과 위치 검증을 대신하지 않는다.
...
- `Value=0x157`, `Offset=0`: 100 km/h 구간단속에서 확인된 종류 7의 제한속도 구간이다.
  ...
  1959 m와 근접하므로 미래 정확거리 카메라 후보로 보관한다. 현재 카메라 경고가 활성화된 동안에는
  같은 속도라는 이유만으로 이 먼 종점 거리를 현재 경고에 연결하지 않는다. 구간 제한은 별도 상한으로 유지한다.

현재 카메라 경고와 미래 거리의 연결은 경고 시작 시점의 가상거리 끝점에 40m 오프셋 여유를 더한
절대 위치 안에서만 허용한다. 이 위치는 차량 이동에 따라 앞으로 밀리지 않는다. 가까운 일치 후보가
없으면 현재 경고만의 가상거리를 사용하므로 이전 미래 후보의 긴 거리를 물려받지 않는다.
새 경고·제한속도 변경 시 끝점을 다시 잡고, 가상거리 설정 변경도 반영한다. 경로 후보 삭제는 현재 경고의
끝점을 초기화하지 않는다. 실제·가상 거리가 소진되어도 현재 경고가 켜져 있으면 최소 1m를 유지해
카메라 감속 후보를 유지하며, 경고 종료 시 일치했던 이벤트만 제거하고 나머지 미래 후보는 보존한다.
이는 2026-09-13 `carrot-wip` 및 유지 중인 `carrot-*`의 순정 CAN 처리 변경이며 과거 비교 브랜치에는 소급하지 않는다.
...
Live AutoNaviSpeedCtrlMode disable invalidates the cached SDI before candidate
selection. Re-enabling requires a fresh safety projection; old distance is not replayed.
