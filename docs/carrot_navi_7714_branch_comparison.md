...
## Naver 실험 브랜치의 방지턱 입력 보완

정확한 Naver 방지턱 객체와 신선한 양의 거리는 road category 미제공 상태에서도
동일한 CarrotServ 감속 계산을 사용한다. 명시적인 0/1 분류나 설정으로 비활성화된
방지턱은 허용하지 않는다. Tmap의 기존 분류 규칙과 시작·종료·가속페달 계산은 그대로다.
객체 결합은 같은 안내 세션의 단일 소스/최종 객체 쌍만 사용하며 과거 거리를 재사용하지
않는다. 현재 전체 매핑 수준과 실기기 검증 범위는 [대응표](naver/MAPPING_PARITY.md)를 따른다.
