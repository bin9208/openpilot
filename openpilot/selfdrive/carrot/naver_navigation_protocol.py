...
def _parse_route(value: object, received_mono_s: int | float) -> tuple[
  bool, int | float | None, int | None, int | float, int | float, bool, tuple[float, float] | None,
  ...
  if not present:
    ...
    return False, None, None, 0.0, 0.0, False, None, ()
  ...
  if _ROUTE_REVISION_KEY in route:
    expected |= frozenset((_ROUTE_REVISION_KEY,))
  _exact_keys(route, expected, "route")

  revision = (
    None if _ROUTE_REVISION_KEY not in route
    else _integer_field(route, _ROUTE_REVISION_KEY, 0, MAX_SEQUENCE, "route")
  )
  ...
  return True, received_mono_s, revision, remaining_distance, remaining_time, off_route, destination, points
