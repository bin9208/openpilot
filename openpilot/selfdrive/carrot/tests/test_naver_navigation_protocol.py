...
def test_parses_complete_naver_frame_without_synthetic_defaults():
  ...
  assert parsed.control.route_revision == 7
...
def test_rejects_every_missing_required_key_in_each_nested_exact_schema(schema_name):
  ...
  for missing_key in (key for key in tuple(target) if key not in ("points", "revision")):
    bad = deepcopy(frame)
...
def test_present_route_without_revision_remains_v1_compatible():
  frame = valid_frame()
  del frame["route"]["revision"]

  parsed = parse_naver_navigation_v1(frame, 10.0)

  assert parsed.control.route_present
  assert parsed.control.route_revision is None
  assert parsed.control.route_received_mono_s == 10.0


@pytest.mark.parametrize("revision", [-1, MAX_SEQUENCE + 1, 1.5, True, "7"])
def test_rejects_invalid_route_revision(revision):
  frame = valid_frame()
  frame["route"]["revision"] = revision
  assert_protocol_error(frame)


def test_absent_route_rejects_revision_contradiction():
  frame = valid_frame()
  frame["route"] = {"present": False, "revision": 7}
  assert_protocol_error(frame)
