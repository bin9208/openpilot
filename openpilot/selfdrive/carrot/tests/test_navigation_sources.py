...
def test_unchanged_route_revision_does_not_create_activation():
  store = NavigationSourceStore()
  first_control = NavigationControlState(
    route_present=True,
    route_revision=7,
    route_received_mono_s=1.0,
    route_points=((37.5, 127.1),),
  )
  assert store.accept(_snapshot(
    NavigationSource.NAVER_V1, "stable-route", 1, 1.0, control=first_control,
  ), 1.0)
  first = store.select(1.0)
  assert first.snapshot is not None
  activation_epoch = first.snapshot.activation_epoch

  heartbeat_control = replace(first_control, route_received_mono_s=1.5)
  assert store.accept(_snapshot(
    NavigationSource.NAVER_V1, "stable-route", 2, 1.5, control=heartbeat_control,
  ), 1.5)
  heartbeat = store.select(1.5)
  assert heartbeat.snapshot is not None
  assert heartbeat.snapshot.activation_epoch == activation_epoch
  assert heartbeat.snapshot.control.route_revision == 7


def test_route_revision_does_not_change_transport_loss_lease_behavior():
  store = NavigationSourceStore()
  control = NavigationControlState(
    route_present=True,
    route_revision=7,
    route_received_mono_s=5.0,
  )
  assert store.accept(_snapshot(
    NavigationSource.NAVER_V1, "revision-lease", 1, 5.0, control=control,
  ), 5.0)
  assert store.record_transport_loss(NavigationSource.NAVER_V1, "revision-lease", 5.1)

  selected = store.select(6.999)
  assert selected.snapshot is not None
  assert selected.snapshot.control.route_revision == 7
...
def test_route_revision_does_not_revive_terminal_session(lifecycle):
  store = NavigationSourceStore()
  control = NavigationControlState(
    route_present=True,
    route_revision=7,
  ...
  assert store.accept(_snapshot(
    NavigationSource.NAVER_V1, "revision-ended", 1, 5.0, control=control,
  ), 5.0)
  assert store.accept(_terminal(
    NavigationSource.NAVER_V1, "revision-ended", 2, 5.1, lifecycle,
  ), 5.1)
  assert not store.accept(_snapshot(
    NavigationSource.NAVER_V1,
    "revision-ended",
    3,
    5.2,
    control=replace(control, route_revision=8, route_received_mono_s=5.2),
  ), 5.2)
